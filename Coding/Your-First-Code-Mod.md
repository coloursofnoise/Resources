...
:warning: This one-line method might not work in all cases, especially if the vanilla routine is a "state machine" routine that changes the state (`StateMachine.State = xx`). In this case, any code you insert **after** the coroutine ends isn't run. You can use this instead:
```cs
